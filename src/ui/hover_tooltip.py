...
def install_hover_tooltip(canvas, figure) -> None:
    ...
    """
    _state: dict = {"annot": None, "last_label": ""}
    ...
    def _on_move(event):
        ...
        # Check all axes sharing the same position (e.g. twinx pairs),
        # not just the topmost one returned by event.inaxes.
        axes_to_check = [a for a in figure.axes if a.bbox.bounds == ax.bbox.bounds]
        if not axes_to_check:
            axes_to_check = [ax]

        for check_ax in axes_to_check:
            for artist in check_ax.get_children():
                try:
